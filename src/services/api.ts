...
export const api = {
  ...
  async createOrder(order: Omit<OrderItem, 'id' | 'criadoEm'>): Promise<OrderItem | null> {
    ...
    try {
      ...
      /**
       * FIX APPLIED BELOW:
       * The error shown in the user's image (ts(2783)) occurs because specific fields 
       * (idPedido, horaPedido, etc) were defined, and then `...payload` was spread afterwards,
       * but TypeScript treats the spread of a cast type (BackendOrder) as overriding the 
       * previous keys.
       * 
       * Solution: Spread `payload` FIRST. Explicit keys override the spread.
       */
      const newBackendOrder: BackendOrder = {
        // 1. Spread payload (cast as Partial or just the object) first to allow defaults
        ...(payload as unknown as BackendOrder), 
        
        // 2. Explicitly overwrite generated fields. This order satisfies TS that these
        // specific values are the ones we intend to keep.
        idPedido: mockId,
        horaPedido: now,
        precoTotal: frontendPrice,

        // 3. Fallback defaults if payload didn't provide them (Safety for BackendOrder type)

...
function App() {
  ...
  const handleConfirmOrder = async (orderData: Omit<OrderItem, 'id' | 'criadoEm'>) => {
    ...
    // Create New Order via API (Saves to SQL and CSV)
  ...
  const handleCheckout = async () => {
    // Processa cada item como "Finalizado"
    // Isso remove do SQL de Ativos, Remove do CSV de Ativos e Adiciona no CSV de Histórico
    
    if (orders.length === 0) return;

    try {
      // Executa a finalização de todos os pedidos em paralelo
      const promises = orders.map(order => api.deleteOrder(order.id));
      await Promise.all(promises);
      
      alert(`Sucesso!\n\n${orders.length} pedidos foram finalizados e registrados no histórico.`);
      setOrders([]); // Limpa a lista local
      setIsDrawerOpen(false);
    } catch (error) {
      console.error("Erro no checkout", error);
      alert("Ocorreu um erro ao finalizar alguns pedidos.");
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className={`relative p-2 rounded-full text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition-all duration-300 group
                   ${cartBump ? 'scale-110 text-primary bg-orange-50 dark:bg-gray-800' : ''}
                `}
              >
                <ShoppingCart size={22} className={`transition-colors ${cartBump ? 'text-primary' : 'group-hover:text-primary'}`} />
                {orders.length > 0 && (
                  <span className={`absolute top-0 right-0 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-white transform translate-x-1/4 -translate-y-1/4 bg-primary rounded-full border-2 border-white dark:border-gray-900 transition-transform duration-300 ${cartBump ? 'scale-125' : 'scale-100'}`}>
                    {orders.length}
                  </span>
                )}
              </button>
